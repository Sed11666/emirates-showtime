...
/* ── Cine Royal screen types ──────────────────────────────
 *
 * cinemauae reports every Cine Royal screening as "Regular", so all 800 of
 * them rendered as Standard and a Royal Class recliner looked like an ordinary
 * seat. Cine Royal's own showtimes endpoint groups its sessions by screenType
 * — STANDARD, ROYAL CLASS, ROYAL KIDS — so one call per film per day recovers
 * what the aggregator flattens.
 *
 * This is the same endpoint the Cine Royal deep-link investigation found. It
 * cannot give us a per-screening booking URL (there is no per-show id; see
 * CLAUDE-HANDOVER §11.1), but the screen metadata is real and worth having.
 *
 * Enrichment only. A failure here must never fail the page: worst case the
 * screenings keep the format cinemauae gave them, which is what they had
 * before.
 */
const CINEROYAL_API = "https://cineroyal.ae/home/getShowTimesByFilmAndShowDate";

/**
 * Their venue names are not ours. Most differ only by the trailing "Cinema",
 * which normalising removes, but one is a genuine rename and has to be stated.
 */
const CINEROYAL_VENUE_ALIASES: Record<string, string> = {
  "wtc mall abu dhabi": "world trade center",
};

function crVenueKey(name: string): string {
  const value = String(name ?? "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/ cinema$/, "")
    .trim();
  return CINEROYAL_VENUE_ALIASES[value] ?? value;
}

/** "05:30PM" -> "17:30". Returns null rather than guessing at a bad value. */
function crTo24h(value: string): string | null {
...
/** "2026-08-27" -> "27-Aug-2026", the only date format their endpoint takes. */
function crShowDate(dayKey: string): string | null {
...
/** One request answers a whole film-day, so results are cached for the run. */
const crCache = new Map<string, Map<string, string>>();

async function crScreenTypes(prettyUrl: string, dayKey: string): Promise<Map<string, string>> {
  const cacheKey = `${prettyUrl}|${dayKey}`;
  const cached = crCache.get(cacheKey);
  if (cached) return cached;

  const out = new Map<string, string>();
  const showdate = crShowDate(dayKey);
  if (!showdate) return out;

  try {
    const res = await fetch(CINEROYAL_API, {
      method: "POST",
      headers: {
        "content-type": "application/json;charset=UTF-8",
        accept: "application/json",
        referer: `https://cineroyal.ae/home/chooseScreen/${prettyUrl}`,
      },
      body: JSON.stringify({
        prettyUrl,
        showdate,
    ...
    const json = (await res.json()) as {
      shows?: Array<{ screenType?: string; filmShowTimeList?: Array<Record<string, unknown>> }>;
    };
    for (const group of json.shows ?? []) {
      const screenType = typeof group.screenType === "string" ? group.screenType.trim() : "";
      if (!screenType) continue;
      for (const show of group.filmShowTimeList ?? []) {
        const venue = typeof show["cinema"] === "string" ? show["cinema"] : "";
        const time = crTo24h(String(show["formattedShowTime"] ?? ""));
        if (!venue || !time) continue;
        out.set(`${crVenueKey(venue)}|${time}`, screenType);
      }
    }
  } catch {
    // Their endpoint being slow or down leaves formats as cinemauae gave them.
  }

  crCache.set(cacheKey, out);
  return out;
}

/**
 * Replaces the flattened format on Cine Royal screenings with the real screen
 * type. The film slug comes from the booking URL cinemauae already gives us
 * (`/home/chooseScreen/{slug}`), so nothing has to be guessed or constructed.
 */
async function enrichCineRoyalFormats(screenings: Screening[], dayKey: string): Promise<void> {
  const cineroyal = screenings.filter((s) => s.chainKey === "cineroyal");
  if (cineroyal.length === 0) return;

  const slug = cineroyal
    .map((s) => /\/chooseScreen\/([a-z0-9-]+)/i.exec(s.bookingUrl ?? "")?.[1])
    .find(Boolean);
  if (!slug) return;

  const types = await crScreenTypes(slug, dayKey);
  if (types.size === 0) return;

  for (const screening of cineroyal) {
    const type = types.get(`${crVenueKey(screening.venue)}|${screening.time}`);
    // Through canonicalFormat like every other format: their API shouts
    // "ROYAL CLASS", and storing that raw would undo the canonical casing that
    // keeps "Gold" and "GOLD" from being two products.
    if (type) screening.format = canonicalFormat(type);
  }
}
...
async function runScrape(request: Request) {
  ...
  for (const page of ordered) {
    ...
    for (let dayIndex = 0; dayIndex < SCRAPE_DAYS; dayIndex += 1) {
      ...
      await enrichCineRoyalFormats(parsed.screenings, dayKey);
